...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
            ...
            EarlyStop_Patience: int = 9999999,
            ):
        """
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                if evaluation_steps > 0 and training_steps % evaluation_steps == 0: #Este aqui é se eu quisesse dar evaluate tipo a meio de um epoch. Acaba por ser desnecessário
                    self._eval_during_training(evaluator, output_path, save_best_model, epoch, training_steps, callback, EarlyStop_Patience)
            ...
            EarlyStop = self._eval_during_training(evaluator, output_path, save_best_model, epoch, -1, callback, EarlyStop_Patience)
            if(EarlyStop):
                print("Early Stop Was triggered when model ended epoch: {}".format(epoch))
                return
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback, EarlyStop_Patience):
        ...
        if evaluator is not None:
            score, csv_path = evaluator(self, output_path=eval_path, epoch=epoch, steps=steps)
            ...
            if (callback==True):
                ...
                results_csv = results_csv.iloc[:,2]
                if ((len(results_csv) - 1 > np.argmax(results_csv) + EarlyStop_Patience)): #Check if should trigger the EarlyStop
                    return True
                else:
                    return False
            else:
