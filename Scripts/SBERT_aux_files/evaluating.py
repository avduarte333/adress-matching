...
class EmbeddingSimilarityEvaluator(SentenceEvaluator):
    ...
    def __init__(self, sentences1: List[str], sentences2: List[str], scores: List[float], batch_size: int = 16, main_similarity: SimilarityFunction = None, name: str = '', show_progress_bar: bool = False, write_csv: bool = True):
        ...
        self.csv_headers = ["epoch", "steps", "cosine_spearman","BCE"]
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        cosine_probs = (cosine_scores / 2) + 0.5
        loss = torch.nn.BCELoss()
        input = torch.from_numpy(cosine_probs).double()
        target = torch.from_numpy(np.array(labels)).double()
        bce = loss(input, target).item()
        ...
        if output_path is not None and self.write_csv:
            ...
            with open(csv_path, newline='', mode="a" if output_file_exists else 'w', encoding="utf-8") as f:
                ...
                writer.writerow([epoch, steps, eval_spearman_cosine, bce])


        if self.main_similarity == SimilarityFunction.COSINE:
            return (eval_spearman_cosine, csv_path)
        else:
